...
static char rcsid[] UTIL_UNUSED = "$Id: TypeChecker.c,v 1.1.2.56.4.3 2007/03/20 19:30:09 nusmv Exp $";
...
******************************************************************************/
SymbTable_ptr TypeChecker_get_symbol_table(TypeChecker_ptr self)
{
  TYPE_CHECKER_CHECK_INSTANCE(self);
				...
				SymbLayer_ptr layer)
{
  ...
  table = TypeChecker_get_symbol_table(self);
  ...
  while (!ListIter_is_end(iter)) {
    node_ptr varName = NodeList_get_elem_at(vars, iter);
  ...
  {
     NodeList_ptr defs = SymbLayer_get_defines(layer);
     ListIter_ptr iter;
     for (iter = NodeList_get_first_iter(defs); !ListIter_is_end(iter);
	  iter = ListIter_get_next(iter)) {
       node_ptr defName = NodeList_get_elem_at(defs, iter);
				   ...
				   Prop_ptr property)
{
  ...
  case Prop_ReachTarget: kind = REACHTARGET; break;
  case Prop_AvoidTarget: kind = AVOIDTARGET; break;
  case Prop_BuchiGame: kind = BUCHIGAME; break;
  case Prop_GenReactivity: kind = GENREACTIVITY; break;
...
/**Function********************************************************************
  ...
  Description        [
  The main purpose of this function is to be invoked on temprorarily
...
/**Function********************************************************************
  ...
  The constrains on a type are:
     1. word type: the width should be a NUMBER and have positive value.
        a) the width should be greater than implemenetation limit
				       ...
				       node_ptr varName)
{
  ...
  case SYMB_TYPE_WORD: {
    ...
    if (0 < width && width <= WordNumber_max_width()) return true;
  ...
  default:
    nusmv_assert(false); /* a variable cannot have these types */
  }
...
{				      
  ...
  if (node_get_lineno(expr)) fprintf(nusmv_stderr, ", line %d",
				     node_get_lineno(expr));
  fprintf(nusmv_stderr,  ") : ");
}
