...
static char rcsid[] UTIL_UNUSED = "$Id: dd.c,v 1.7.6.12.2.1.2.6.4.12 2007/05/03 07:36:56 nusmv Exp $";
...
add_ptr add_monadic_apply(DdManager * dd, NPFNN/*NPFCVT*/ op, add_ptr f)
{
  ...
  result = Cudd_addApply(dd, op, (DdNode *)f, (DdNode *) one);
...
bdd_ptr bdd_get_one_sparse_sat(DdManager * dd, bdd_ptr d) {
  ...
  if (bdd_is_one(dd, d)) return bdd_one(dd);
  if (bdd_is_zero(dd, d)) return bdd_zero(dd);
