...
static char rcsid[] UTIL_UNUSED = "$Id: BeFsm.c,v 1.1.2.5.6.2 2007/04/30 11:52:55 nusmv Exp $";
...
******************************************************************************/
BeFsm_ptr BeFsm_create_from_sexp_fsm(BeEnc_ptr be_enc, const SexpFsm_ptr bfsm)
{
  node_ptr list_of_valid_fairness = 
    Bmc_CheckFairnessListForPropositionalFormulae(SexpFsm_get_justice(bfsm));
  
  BeFsm_ptr self = 
    BeFsm_create(be_enc, 
                 Bmc_Conv_Bexp2Be(be_enc, SexpFsm_get_init(bfsm)),
                 Bmc_Conv_Bexp2Be(be_enc, SexpFsm_get_invar(bfsm)),
                 Bmc_Conv_Bexp2Be(be_enc, SexpFsm_get_trans(bfsm)),
