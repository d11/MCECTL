...
static be_ptr last_g ARGS((const BeEnc_ptr be_enc,node_ptr ltl_wff,
                           hashPtr table,const int l,const int k,
                           const unsigned pastdepth)); 
                             ...
                             const int time, const int k, const int l)
{
  ...
  if (!Bmc_Utils_IsAllLoopbacks(l)) {
    ...
    tableau = get_f_at_time(be_enc, ltl_wff, formulatable, 0, k, l, 0);
                           ...
                           hashPtr table)
{
  ...
  if (!opt_bmc_sbmc_il_opt(options) || k == 0) { /**normal translation*/    
    be_ptr res; 
    boolean unset = (k==0) && opt_bmc_sbmc_il_opt(options);

    if (unset) unset_bmc_sbmc_il_opt(options);
    res = last_g(be_enc, ltl_wff, table, l, k, pastdepth);
    if (unset) set_bmc_sbmc_il_opt(options);
    return res;
  }
  else { /*il-optimisations enabled*/
    if (node_get_type(ltl_wff) == OP_FUTURE) {
      if (opt_bmc_sbmc_gf_fg_opt(options) &&
       ...
       hashPtr table, const int l, const int k, const unsigned pastdepth)
{
  ...
  nusmv_assert(!opt_bmc_sbmc_il_opt(options) || 
  ...
  if (Bmc_Utils_IsAllLoopbacks(l)) {
    ...
    for (i = k; i--; ) {
      temp=Be_Or(be_mgr, temp,
                 Be_And(be_mgr, 
                        get_g_at_time(be_enc, ltl_wff, table, i+1, k, l,
                                      pastdepth), 
    ...
    /**f(k):=g(l)*/
    return get_g_at_time(be_enc, ltl_wff, table, l, k, l, pastdepth);
  }
  else { /**No loopback*/
    /** f(k):= false*/
...
/**Function********************************************************************
  ...
  Synopsis           [Generate f(k,pastdepth) when pastdepth is less than maximum pastdepth,
  ...
  if (Bmc_Utils_IsAllLoopbacks(l)) {
    ...
    for (i = k; i--; ) {
      temp = 
        Be_Or(be_mgr, temp,
            Be_And(be_mgr,
                   get_f_at_time(be_enc, ltl_wff, table, i+1, k, l, pastdepth),
                   get_el_at_time(be_enc, i, k)));
    }
    return temp;
  }
    ...
    /**f(k):=f1(l)*/
    return get_f_at_time(be_enc, ltl_wff, table, l, k, l, pastdepth);
  }
  else { /**No loopback*/
    /** f(k):= false*/
    return Be_Falsity(be_mgr);
              ...
              const int time, const int k, const int l, const unsigned pastdepth) 
{
  ...
  if (data >= 0) { /**A TL subformula, check cache*/
    be_ptr result = bmc_cache_fetch_f(ltl_wff, time, k, pastdepth, table);
  ...
  case AND:
    return Be_And(be_mgr,
                  get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
                                pastdepth),
                  get_f_at_time(be_enc, cdr(ltl_wff), table, time, k, l,
                                pastdepth));
  case OR:
    return Be_Or(be_mgr,
                 get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
  ...
  case OP_NEXT: { 
    if (time < k) {
      /**f(time):=f(time+1)*/
      return get_f_at_time(be_enc, car(ltl_wff), table, time+1, k, l,
                           pastdepth);
    }
    /**time=k*/
    return last_f(be_enc, car(ltl_wff), table, l, k,
  ...
  case OP_NOTPRECNOT:
  case OP_PREC: 
    return get_ZY_at_time(be_enc, ltl_wff, table, time, k, l, pastdepth);  
  case OP_GLOBAL: 
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_Globally_at_time(be_enc, ltl_wff, table,
                                            time, k, l, pastdepth));
  case OP_FUTURE: 
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
  ...
  case UNTIL: 
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_Until_at_time(be_enc, ltl_wff, table,
                                            time, k, l, pastdepth));
  case RELEASES:
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_V_at_time(be_enc, ltl_wff, table, 
                                            time, k, l, pastdepth));
  case SINCE:
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_Since_at_time(be_enc, ltl_wff, table,
                                            time, k, l, pastdepth));
  case TRIGGERED: 
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
  ...
  case OP_ONCE:  
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_Once_at_time(be_enc, ltl_wff, table,
                                            time, k, l, pastdepth));
  case OP_HISTORICAL: 
    return bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
                              get_Historically_at_time(be_enc, ltl_wff,
                                            table, time, k, l, pastdepth));
...
******************************************************************************/
static be_ptr
get_Globally_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
  ...
  if (time < k) {
    if (opt_bmc_sbmc_gf_fg_opt(options) && 
        (node_get_type(car(ltl_wff)) == OP_FUTURE)) {
      /**f(i):=f(k)*/
      return get_f_at_time(be_enc, ltl_wff, table, k, k, l, pastdepth);
    }
    /*f(time):=f1(time)\wedge f(time+1)*/
    return Be_And(be_mgr, 
                    get_f_at_time(be_enc, car(ltl_wff), table, time, k, l, pastdepth),
                    get_f_at_time(be_enc, ltl_wff, table, time+1, k, l, pastdepth));
  }
  /**time=k*/ 
  if (opt_bmc_sbmc_gf_fg_opt(options) && 
     (node_get_type(car(ltl_wff)) == OP_FUTURE)) {
  ...
  return Be_And(be_mgr,
                get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
                              pastdepth),
                (pastdepth < bmc_past_depth(ltl_wff)) ? 
                last_f(be_enc, ltl_wff, table, l, k, pastdepth + 1) :
...
******************************************************************************/
static be_ptr
get_Eventually_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
...
******************************************************************************/
static be_ptr
get_Until_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l, const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
  ...
  if (time < k) {
    ...
    return Be_Or(be_mgr,
                 ...
                 Be_And(be_mgr,
                        get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
                                      pastdepth),
                        get_f_at_time(be_enc, ltl_wff, table, time+1, k, l,
                                      pastdepth)));
  }     
  /**time=k*/
  ...
  /**pastdepth==n_\psi*/
  return Be_Or(be_mgr,
               get_f_at_time(be_enc, cdr(ltl_wff), table, time, k, l, pd),
               Be_And(be_mgr,
                      get_f_at_time(be_enc, car(ltl_wff), table, time, k, l, pd),
                      last_g(be_enc, ltl_wff, table, l, k, pd)));
}
...
******************************************************************************/
static be_ptr
get_V_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
...
******************************************************************************/
static be_ptr
get_Since_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l, const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
  ...
  if ((time == 0) && (pastdepth == 0)) {
    return get_f_at_time(be_enc, cdr(ltl_wff), table, time, k, l, pastdepth); 
  }
  else if ((pastdepth == 0) && (time > 0) && (time <= k)) {
    /*f(i):=f2(i)\vee (f1(i) \wedge f(i-1))*/
    return Be_Or(be_mgr,
                 get_f_at_time(be_enc, cdr(ltl_wff), table, time, k, l, pastdepth),
                 Be_And(be_mgr,
                        get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
                                      pastdepth),
                        get_f_at_time(be_enc, ltl_wff, table, time-1, k, l,
    ...
    return Be_Or(be_mgr,
                 ...
                 Be_And(be_mgr,
                        ...
                        Be_Ite(be_mgr,
                               ...
                               get_f_at_time(be_enc, ltl_wff, table, k, k, l,
                                             pastdepth-1),
                               get_f_at_time(be_enc, ltl_wff, table, time-1, k, l,
                                             pastdepth))));
  } 
...
******************************************************************************/
static be_ptr
get_Trigger_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
...
******************************************************************************/
static be_ptr
get_Historically_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
  ...
  if ((time == 0) && (pastdepth == 0)) {
    /**f1(i)*/
    return get_f_at_time(be_enc, car(ltl_wff), table, time, k, l, pastdepth);   
  }
  else if ((pastdepth == 0) && (time > 0) && (time <= k)) {
    ...
    return Be_And(be_mgr,
                  get_f_at_time(be_enc, car(ltl_wff), table, time, k, l,
                                pastdepth),
                  get_f_at_time(be_enc, ltl_wff, table, k, k, l, pastdepth-1));
  }
  nusmv_assert(false);
  return 0; /**Should not be reached*/
}
...
******************************************************************************/
static be_ptr
get_Once_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
...
static be_ptr
get_ZY_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
               const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
  ...
  if ((time == 0) && (pastdepth == 0)) {
    ...
    /**f(i):=f1(i-1)*/
    return get_f_at_time(be_enc, car(ltl_wff), table, time-1, k, l, pastdepth);
  }
  else if ((pastdepth > 0) && (time > 1) && (time < k+1)) {
    /**f(i,d):=el(i)->f1(i-1,d), f(k,d-1)*/
    return Be_Ite(be_mgr,
           get_el_at_time(be_enc, time-1, k),
           get_f_at_time(be_enc, car(ltl_wff), table, k, k, l, pastdepth-1),
           get_f_at_time(be_enc, car(ltl_wff), table, time-1, k, l, pastdepth));
  }
  else if ((pastdepth > 0) && (time == 1)) {
    return get_f_at_time(be_enc, car(ltl_wff), table, k, k, l, pastdepth-1);
  }
  nusmv_assert(false);
  return 0; /**Should not be reached*/
}
...
******************************************************************************/
static be_ptr
get_g_at_time(const BeEnc_ptr be_enc, const node_ptr ltl_wff, hashPtr table,
              const int time, const int k, const int l,
              const unsigned pastdepth)
{
  Be_Manager_ptr be_mgr;
      ...
      return bmc_cache_insert_g(ltl_wff, time, k, pastdepth, table,
                                Be_And(be_mgr,
                                       get_f_at_time(be_enc, car(ltl_wff),
                                                     table, time, k, l, pastdepth),
                                       get_g_at_time(be_enc, ltl_wff, table,
      ...
      return bmc_cache_insert_g(ltl_wff, time, k, pastdepth, table,
                                Be_Or(be_mgr,
                                      ...
                                      Be_And(be_mgr,
                                             get_f_at_time(be_enc, car(ltl_wff),
                                                           table, time, k, l, pastdepth),
                                             get_g_at_time(be_enc, ltl_wff, table, time+1,
                                                           k, l, pastdepth))));
    }     
    /**time=k*/
    return bmc_cache_insert_g(ltl_wff, time, k, pastdepth, table,
                              get_f_at_time(be_enc, cdr(ltl_wff), table, time,
                                            k, l, pastdepth));
  ...
  if (Bmc_Utils_IsAllLoopbacks(l)) {
      ...
      tableau=Be_Or(be_mgr,
                    Be_And(be_mgr,
                           ...
                           get_f_at_time(be_enc, car(car(ltl_wff)), table,
                                         i, k, l, pastdepth) :
                           /**normal case: F p*/
                           get_f_at_time(be_enc, car(ltl_wff), table, i, k,
                                         l, pastdepth)),
                    tableau);
    }
  } 
    ...
    for (i = l; i <= k; i++) {
      tableau=Be_Or(be_mgr,
                    (node_get_type(ltl_wff) == OP_GLOBAL) ?
                    /**we're dealing with GF p, skip to f_p(i)*/
                    get_f_at_time(be_enc, car(car(ltl_wff)), table, i, k,
                                  l, pastdepth) :
                    /**normal case: G p*/
                    get_f_at_time(be_enc, car(ltl_wff), table, i, k,
                                  l, pastdepth),
                             ...
                             const unsigned pastdepth, hashPtr table)
{
  ...
  if (Bmc_Utils_IsAllLoopbacks(l)) {
    ...
    tableau = Be_And(be_mgr, tableau,
                     get_loop_exists(be_enc, k));
  } 
  else if (Bmc_Utils_IsSingleLoopback(l)) {
...
static void bmc_cache_init(const int count, const int k, const unsigned pastdepth)
{
  if (opt_bmc_sbmc_cache(options)) { 
    ...
    */
    ...
    if (opt_bmc_sbmc_il_opt(options)) {
...
{
  nusmv_assert((time >= 1) && (time <= k) && opt_bmc_sbmc_il_opt(options));
...
{
  nusmv_assert((time > 0) && (time <= k) && opt_bmc_sbmc_il_opt(options));
