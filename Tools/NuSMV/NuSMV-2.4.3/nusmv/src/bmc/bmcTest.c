...
static char rcsid[] UTIL_UNUSED = "$Id: bmcTest.c,v 1.20.4.5.2.2.2.5 2006/07/19 14:40:41 nusmv Exp $";
...
int Bmc_TestTableau(int argc, char ** argv)
{
  ...
  /* process command options */
  {
	...
	ltlspec = Compile_FlattenSexpExpandDefine(Compile_get_global_symb_table(), 
						  ltlspec, Nil);
	wff = Bmc_Wff_MkNnf(Compile_detexpr2bexpr(Enc_get_bdd_encoding(), ltlspec));
	break;
  ...
  if (wff == NULL) {
    ...
    case GWO_Globally:
      wff = Bmc_Wff_MkGlobally(bmc_test_gen_wff(be_enc, max_depth, 
						max_conns, usePastOperators));
      break;

    case GWO_Future:
      wff = Bmc_Wff_MkEventually(bmc_test_gen_wff(be_enc, max_depth, max_conns, 
						  usePastOperators));
      break;

    case GWO_Until:
      wff = Bmc_Wff_MkUntil(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
			    bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;

    case GWO_Releases:
      wff = Bmc_Wff_MkReleases(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
			       bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;

    case GWO_Historically:
      wff = Bmc_Wff_MkHistorically(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;

    case GWO_Once:
      wff = Bmc_Wff_MkOnce(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;

    case GWO_Since:
      wff = Bmc_Wff_MkSince(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
			    bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;

    case GWO_Triggered:
      wff = Bmc_Wff_MkTriggered(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
				bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
      break;
  ...
  if (!crossComparison) {
    ...
    wff = Bmc_Wff_MkImplies(bmc_test_mk_loopback_ltl(be_enc, k, l), wff);
...
bmc_test_mk_loopback_ltl(const BeEnc_ptr be_enc, const int k, const int l)
{
  ...
  bigand_vars = Bmc_Wff_MkIff(var, Bmc_Wff_MkXopNext(var, loop_len));
  ...
  while (BeEnc_is_var_index_valid(be_enc, idx)) {
    var = BeEnc_index_to_name(be_enc, idx);
    single_var_eq = Bmc_Wff_MkIff(var, Bmc_Wff_MkXopNext(var, loop_len));
				 ...
				 boolean usePastOperators)
{
  ...
  case 1:
    return Bmc_Wff_MkNot(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 2:
    return Bmc_Wff_MkOr (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                         bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 3:
    return Bmc_Wff_MkAnd(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                         bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 4:
    return Bmc_Wff_MkImplies(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                             bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 5:
    return Bmc_Wff_MkIff(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                         bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  /* Future operators */
  case 6:
    return Bmc_Wff_MkOpNext    (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 7:
    return Bmc_Wff_MkEventually(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 8:
    return Bmc_Wff_MkGlobally  (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 9:
    return Bmc_Wff_MkUntil     (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                                bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
  case 10:
    return Bmc_Wff_MkReleases  (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                                bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  /* Past operators */
  case 11:
    return Bmc_Wff_MkOpPrec      (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 12:
    return Bmc_Wff_MkOnce        (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 13:
    return Bmc_Wff_MkHistorically(bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));

  case 14:
    return Bmc_Wff_MkSince       (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                                  bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
  case 15:
    return Bmc_Wff_MkTriggered   (bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators),
                                  bmc_test_gen_wff(be_enc, max_depth, max_conns, usePastOperators));
