...
/**CFile***********************************************************************
  ...
  Revision    [$Id: PrinterBase.c,v 1.1.2.5 2006/07/12 12:39:11 nusmv Exp $]
...
static char rcsid[] UTIL_UNUSED = "$Id: PrinterBase.c,v 1.1.2.5 2006/07/12 12:39:11 nusmv Exp $";
...
PrinterBase_create(const char* name, int low, size_t num)
{
  ...
  printer_base_init(self, name, low, num);
  return self;
}
		       ...
		       int low, size_t num)
{
  ...
  node_walker_init(NODE_WALKER(self), name, low, num, true);
