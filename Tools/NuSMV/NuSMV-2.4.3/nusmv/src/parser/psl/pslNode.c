...
static char rcsid[] UTIL_UNUSED = "$Id: pslNode.c,v 1.1.4.18.4.5 2007/03/27 09:50:51 nusmv Exp $"; 
...
boolean psl_node_is_leaf(PslNode_ptr expr)
{
  ...
  return (psl_node_get_op(expr) == NUMBER || 
	  psl_node_get_op(expr) == NUMBER_REAL || 
	  psl_node_get_op(expr) == ATOM ||
	  psl_node_get_op(expr) == TRUEEXP ||
	  psl_node_get_op(expr) == FALSEEXP);
}
...
PslOp psl_node_get_replicator_join_op(PslNode_ptr _repl)
{
  ...
  default: nusmv_assert(false); /* no other possible cases */
  }
  return -1;
}
...
static boolean psl_node_is_star_free(PslNode_ptr expr)
{
  ...
  /* leaf? */
  if (psl_node_is_id(expr) || op==NUMBER || op==NUMBER_REAL) return true; 
