...
static char rcsid[] UTIL_UNUSED = "$Id: error.c,v 1.8.2.14.4.31.4.4 2007/04/05 15:42:49 nusmv Exp $";
...
{
  start_parsing_err();
  fprintf(nusmv_stderr, "the width of the Word variable ");
  print_node(nusmv_stderr, name);
...
{
  start_parsing_err();
  fprintf(nusmv_stderr, "at least on of the width of the Word Array variable ");
  print_node(nusmv_stderr, name);
  fprintf(nusmv_stderr, " is not a constant");  
  finish_parsing_err();
}
...
{
  start_parsing_err();
  fprintf(nusmv_stderr, "variable \"");
  print_node(nusmv_stderr, s);
  fprintf(nusmv_stderr, "\" shadows formal parameter");
  finish_parsing_err();
}
...
void error_assign_exp_contains_input_vars(node_ptr exp)
{
  ...
  fprintf(nusmv_stderr, " contains illegal input variables");
  finish_parsing_err();
}
