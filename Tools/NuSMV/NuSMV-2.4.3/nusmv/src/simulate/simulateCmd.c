...
static char rcsid[] UTIL_UNUSED = "$Id: simulateCmd.c,v 1.14.2.26.4.8.4.3 2006/11/16 14:54:29 nusmv Exp $";
...
int CommandSimulate(int argc, char **argv)
{
  ...
  else bdd_constraints = bdd_one(dd);
					   ...
					   boolean only_changes)
{
  ...
  if (printyesno) {
    ...
    TracePlugin_action(plugin, trace, 
		       TraceManager_get_registered_layers(global_trace_manager), 
		       iter);
  }
