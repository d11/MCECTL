...
/***************************************************************************
  ...
  ------------
    ...
    The class specified as the template parameter must have the following 
    operators defined:
*******************************
...
public:
	...
	int		operator==(const pool<S>&) const;
 			...
 			//  equal (managing the identical set of arrays) 
                        //  to the parameter pool and false otherwise.
	...
	void*		get_member();
			...
			//  allocated from one of the free arrays if possible,
			//  but otherwise a new array which is twice the
			//  size of the last array is allocated.
