...
#define SignedEdge(E,S) (Dag_Vertex_t *) (((nusmv_ptrint)(EnDualNode(E))&DAG_BIT_CLEAR)|S)
...
DFS2( Dag_Vertex_t * v, nusmv_ptrint vBit, lsList sons, int father_symbol)
{
 ...
 if (vBit!=0                    || /* edge is negated */
     ...
     if (!Compare (v,sons,vBit))
         (void) lsNewEnd(sons, (lsGeneric)(((nusmv_ptrint)(v)&DAG_BIT_CLEAR)|vBit), LS_NH);
     return;
 }
...
Compare(Dag_Vertex_t* v, lsList list, nusmv_ptrint vBit)
{
    ...
    while (lsNext(gen, (lsGeneric*) &h, LS_NH) == LS_OK)
        if ((lsGeneric)(((nusmv_ptrint)(v)&DAG_BIT_CLEAR)|vBit)==(lsGeneric)(((nusmv_ptrint)(h)&DAG_BIT_CLEAR)|Dag_VertexIsSet(h)) )
            return (1);
