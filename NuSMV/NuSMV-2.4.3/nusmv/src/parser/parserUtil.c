...
static char rcsid[] UTIL_UNUSED = "$Id: parserUtil.c,v 1.7.4.6.4.14.4.1 2007/05/09 13:13:04 nusmv Exp $";
...
static boolean parser_parse_real_flag = 1;
...
{
  int retval = 0;
  if (strcmp(get_pp_list(options), "") != 0) parser_open_input_pp(filename);
  ...
  FAIL {
    if (strcmp(get_pp_list(options), "") != 0) parser_close_input_pp();
...
int Parser_ReadCmdFromFile(const char *filename)
{
  int status;
  char * old_input_file;
  ...
  parse_mode_flag = PARSE_COMMAND;
  old_input_file = get_input_file(options);
  set_input_file(options, (char*)(NULL == filename ? "<stdin>" : filename));
...
/*---------------------------------------------------------------------------*/
void parser_parse_real_set() {
  parser_parse_real_flag = true;
}

void parser_parse_real_unset() {
  parser_parse_real_flag = false;
}

boolean parser_parse_real() {
  return parser_parse_real_flag;
}
