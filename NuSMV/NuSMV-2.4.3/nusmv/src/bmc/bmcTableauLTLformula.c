...
static char rcsid[] UTIL_UNUSED = "$Id: bmcTableauLTLformula.c,v 1.3.4.5.2.2.2.5 2005/04/05 15:52:12 nusmv Exp $";
			 ...
			 const int time, const int k, const int l)
{
  ...
  case TRUEEXP:
    result = Be_Truth(be_mgr); break;

  case FALSEEXP:
    result = Be_Falsity(be_mgr); break;
  ...
  case DOT:
    ...
    result = BeEnc_name_to_timed(be_enc, ltl_wff, time);
    break;
