...
static char rcsid[] UTIL_UNUSED = "$Id: bmcWff.c,v 1.12.4.5.2.3.2.4 2005/07/01 17:23:30 nusmv Exp $";
...
static node_ptr bmc_wff_expand_case_aux(node_ptr wff) 
{
  if (node_get_type(wff) == CASE) {
    ...
    */
    if (cur_cond == Bmc_Wff_MkTruth()) return cur_rslt;
    if (cur_cond == Bmc_Wff_MkFalsity()) return case_rest;
    ...
    if (FAILURE == node_get_type(wff)) {
      report_failure_node(wff); /* failure node found */
    }
