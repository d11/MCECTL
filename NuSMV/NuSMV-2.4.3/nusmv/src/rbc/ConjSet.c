/**CFile***********************************************************************
  ...
  Revision    [$Id: ConjSet.c,v 1.1.2.7 2007/05/16 15:14:44 nusmv Exp $]
...
static char rcsid[] UTIL_UNUSED = "$Id: ConjSet.c,v 1.1.2.7 2007/05/16 15:14:44 nusmv Exp $";
...
typedef struct ConjElem_TAG
{
  ...
  lsList deps;  /* unflattened expression dependencies */
} ConjElem;
...
static void conj_set_flattenize(ConjSet_ptr self, Rbc_t* var, hash_ptr pvars)
{
  ...
  if (lsLength(el->deps) == 0) {
    el->fexpr = el->expr;    
  }
  else { /* resolves first all dependencies */
    lsGen gen; lsGeneric data;
    
    lsForEachItem(el->deps, gen, data) {
...
static ConjElem_ptr ConjElem_copy(const ConjElem_ptr self)
{
  ...
  copy->deps = lsCopy(self->deps, NULL);  
...
{
  CONJ_ELEM_CHECK_INSTANCE(self);
  return lsLength(self->deps) < lsLength(other->deps);
}
