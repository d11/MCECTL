...
static char rcsid[] UTIL_UNUSED = "$Id: rbcSubst.c,v 1.3.6.2.2.1.2.5.6.5 2007/04/04 12:00:14 nusmv Exp $";
...
/**Function********************************************************************
  ...
  Description [Given `rbcManager', the rbc `f', and the array of integers
	       ...
	       Here the substitution is performed completely at physical 
	       level (i.e. at the level of pure rbc indices). For a substitution 
	       at logical level, see Rbc_LogicalSubst.
  ...
  if (f -> symbol == RBCVAR) {
    ...
    sd -> result = 
      Rbc_GetIthVar(sd -> rbcManager,
		    ((int*)sd -> subst)[PTR_TO_INT(f -> data)]);
  }
  else {
...
static void LogicalSubstLast(Rbc_t* f, char* SubstData, nusmv_ptrint sign)
{
  ...
  if (f -> symbol == RBCVAR) {
    ...
    sd -> result = Rbc_GetIthVar(sd->rbcManager, 
		     sd->log2phy[sd->subst[sd->phy2log[PTR_TO_INT(f -> data)]]]);
  }
  else {
    /* Substitutions may trigger simplifications. */
    if(f -> symbol == RBCAND)
...
******************************************************************************/
static void LogicalSubstRbcLast(Rbc_t* f, char* SubstRbcData, nusmv_ptrint sign)
{
  SubstRbcDfsData_t * sd   = (SubstRbcDfsData_t*)SubstRbcData;
  Rbc_t         ** sons = (Rbc_t**)(f -> gRef);
