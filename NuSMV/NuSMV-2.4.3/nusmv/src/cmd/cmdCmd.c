 ...
 */
static char rcsid[] UTIL_UNUSED = "$Id: cmdCmd.c,v 1.8.2.1.4.4.4.2 2007/05/09 14:24:36 nusmv Exp $";
...
sigterm(int sig)
{
  ...
  if (!opt_batch(options) && !cmd_is_curr_reentrant()) {
    fprintf(nusmv_stderr, 
	    "Warning: %s status may be not consistent. Use 'reset' "\
	    "command if needed.\n", get_pgm_name(options));
  }
