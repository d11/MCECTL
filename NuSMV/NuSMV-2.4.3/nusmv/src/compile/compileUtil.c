...
static char rcsid[] UTIL_UNUSED = "$Id: compileUtil.c,v 1.29.4.12.4.8.4.9 2007/05/15 14:24:49 nusmv Exp $";
...
node_ptr Compile_pop_distrib_ops(node_ptr prop) {
  ...
  if (opt_verbose_level_gt(options, 6)) {
    if (prop == result) {
     fprintf(nusmv_stderr, "-- No simplifcation occured\n");
    }
    else {
					 ...
					 const NodeList_ptr vars_order)
{
  ...
  iter = NodeList_get_first_iter(vars_order);
  while (!ListIter_is_end(iter)) {
    node_ptr name = CompileFlatten_resolve_name(st, 
			       NodeList_get_elem_at(vars_order, iter), Nil);

    if (NodeList_belongs_to(vars, name) && !NodeList_belongs_to(res, name)) {
  ...
  while (!ListIter_is_end(iter)) {
    ...
    if (!NodeList_belongs_to(res, name)) NodeList_append(res, name);
    iter = ListIter_get_next(iter);
  }
...
int Compile_check_if_model_was_built(FILE* err, boolean forced)
{
  ...
  if (cmp_struct_get_build_model(cmps) == 0 && 
      opt_cone_of_influence(options) && !forced) return 0;
