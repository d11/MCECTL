/**CFile***********************************************************************
  ...
  Revision    [$Id: CheckerPsl.c,v 1.1.2.7.4.4 2007/04/04 12:00:33 nusmv Exp $]
...
static char rcsid[] UTIL_UNUSED = "$Id: CheckerPsl.c,v 1.1.2.7.4.4 2007/04/04 12:00:33 nusmv Exp $";
					   ...
					   node_ptr e, node_ptr ctx)
{
  ...
  case PSL_SERECOMPOUND:
	...
	return _SET_TYPE(ctx_expr, SymbTablePkg_error_type()); 
      ...
      if (SymbType_is_boolean_enum(type)) return _SET_TYPE(ctx_expr, type);
  ...
  case PSL_CONCATENATION:
	...
	if (!SymbType_is_boolean_enum(right)) {
	...
	is_ok = SymbType_is_boolean_enum(left);
  ...
  case PSL_SEREREPEATED:
	...
	if (tcount != SymbTablePkg_integer_type() &&
	    tcount != SymbTablePkg_boolean_enum_type()) is_ok = false;
      }
	...
	if (SymbType_is_boolean_enum(tsere)) {
	  /* ok */
	  return _SET_TYPE(ctx_expr, SymbTablePkg_boolean_enum_type());
	}
      }
  ...
  case PSL_REPLPROP:
	...
	TypeChecker_get_symbol_table(TYPE_CHECKER(NODE_WALKER(self)->master));
	...
	psl_node_get_replicator_normalized_value_set(repl);
      ...
      /* contextualized id name */
      node_ptr id_ctx = CompileFlatten_resolve_name(symb_table, 
	...
	is_ok = SymbType_is_boolean_enum(prop_type);
  ...
  case PSL_WITHIN_:
	...
	return _SET_TYPE(ctx_expr, SymbTablePkg_error_type()); 
      ...
      if (SymbType_is_boolean_enum(t1) && SymbType_is_boolean_enum(t2) && 
	  SymbType_is_boolean_enum(t3)) {
	return _SET_TYPE(ctx_expr, SymbTablePkg_boolean_enum_type());
      }
  ...
  case PSL_XBANG:
	...
	return _SET_TYPE(ctx_expr, SymbTablePkg_error_type()); 
      ...
      if (SymbType_is_boolean_enum(t1) && 
	  (t3 == SYMB_TYPE(NULL) || SymbType_is_boolean_enum(t3))) {
	switch (psl_node_get_op(expr)) {
	case PSL_NEXTBANG:
	case PSL_NEXT:
	case PSL_X:
			 ...
			 TypeSystemViolation violation, node_ptr expression)
{
  ...
  */
  ...
  default: 
    nusmv_assert(false); /* unknown kind of error */
